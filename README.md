...
Medallion-style batch pipeline** (bronze → silver → gold) running daily via GitHub Actions, ingesting from Arctic Shift (a community-run Reddit archive) and classifying through Groq's free tier — total infra cost is $0/month, no API approval gating.
...
cp .env.example .env                                   # fill in Groq creds
